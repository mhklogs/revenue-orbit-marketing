...
const Aurora = forwardRef<HTMLDivElement, AuroraProps>(
  (
    {
      colorStops = ["#0F6B63", "#2EC4B0", "#0B3F3C"],
      amplitude = 1.0,
      blend = 0.5,
      time = undefined,
      speed = 1.0,
    ...
    useEffect(() => {
      ...
      const update = (t: number) => {
        ...
        const { time: _time, speed: _speed } = propsRef.current;
        const tValue = _time ?? t * 0.01;
        if (program) {
          program.uniforms.uTime.value = tValue * (_speed ?? speed) * 0.1;
