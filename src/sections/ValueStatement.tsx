...
import {
  Users, Megaphone, TrendingUp, Bot, Settings, Building2, BarChart3,
} from "lucide-react";
  ...
  return (
    <section className="py-16 md:py-24 relative w-full flex flex-col items-center justify-center mx-auto" style={{ backgroundColor: "var(--bg-secondary)" }}>
      <div className="w-full max-w-7xl mx-auto px-6 lg:px-12 text-center">
        <FadeIn className="text-center mx-auto w-full max-w-4xl mb-16">
          <h2 className="text-center mx-auto max-w-4xl text-3xl sm:text-4xl lg:text-5xl font-extrabold tracking-tight leading-snug mb-6 text-white">
            One Partner. <span className="gradient-text">Multiple Growth Engines.</span>
          </h2>
          <p className="text-center mx-auto max-w-2xl text-lg lg:text-xl leading-relaxed text-neutral-200 mb-16">
        ...
        <StaggerContainer className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 w-full max-w-7xl mx-auto justify-center items-stretch">
          {engines.map((e) => (
            <StaggerItem key={e.title}>
              <div className="p-8 lg:p-10 rounded-2xl glass border border-[var(--border-subtle)] hover-lift group h-full flex flex-col gap-5 text-center items-center justify-between">
